...
봇 개선 제안 스키마처럼 새 마이그레이션이 포함된 업데이트라면 먼저 운영 DB를
백업하고 검증 결과를 확인한다. 자세한 절차는 `docs/backup-and-restore.md`를 따른다.
...
`slack-app-manifest.yaml` 변경을 Slack API의 App Manifest 화면에 적용하고 앱을
다시 설치한다. `/제안`은 기존 `commands` scope와 Interactivity 및 Socket Mode를
사용하므로 별도 Request URL이나 OAuth scope는 필요하지 않다. 실제 토큰이나 채널
ID를 문서·로그에 남기지 않는다.
...
9. 일반 채널에서 `/제안`을 실행하면 분류와 제안 내용 모달이 열리고 제출 완료 안내가 표시된다.
10. 관리자 채널에 제안 알림이 도착하며, 관리자 웹 `봇 개선 제안` 탭에서 목록·상세와 `접수` 상태를 확인한다.
11. 제안 상태를 `처리 중`, `완료`로 변경하고 새로고침 후 유지되는지 확인한다.

관리자 알림이 실패하더라도 사용자는 제안이 SQLite에 저장됐다는 안내를 받아야
하며, 대시보드에서 해당 제안을 확인할 수 있어야 한다. DB 저장이 실패하면 모달을
닫지 않고 입력 내용을 유지한 채 재시도 안내를 표시하며 관리자 알림을 보내지 않는다.
...
마이그레이션 11부터 새로 발송하는 운영 회차 공지의 채널·메시지 ID와
발송 본문을 DB에 저장한다. 스케줄러는 60초마다 현재 DB 마감을 확인해
제출 버튼을 제거하고 `회고 제출이 마감되었습니다.`로 표시한다.
원래 본문은 유지하며, 실패 시 다음 주기에 재시도하고 재시작 후에도
미처리 공지를 처리한다. 테스트 회차 공지는 이 처리 대상이 아니다.

이전 버전에서 발송한 공지는 메시지 ID가 없어 자동 수정하지 않는다.
남아 있는 버튼도 제출 진입 단계의 기간 검증으로 차단된다.
이 스키마 변경을 배포하기 전에는 `scripts/backup_database.sh`로 백업하고
검증 결과를 확인한다. 절차는 `docs/backup-and-restore.md`를 따른다.
