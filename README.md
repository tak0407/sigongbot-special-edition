...
추석이나 설 연휴로 한 주 쉬어갈 때는 관리자 웹 `회차 일정` 탭의 `연휴로 회차 미루기`를 씁니다. 미룰 첫 회차와 주 수를 고르면 그 회차부터 마지막 회차까지 한꺼번에 밀립니다. 회차 이름과 순서, 회차 사이 간격은 그대로고 기수만 그만큼 늦게 끝나므로, 쉬어간 만큼 뒤에 회차를 더 붙이고 싶으면 `회차 추가`로 추가 회차를 만듭니다. 회차 이름은 회고 행에 그대로 기록되는 값이라 미룰 때도 바꾸지 않습니다.
...
미루기를 되돌리려면 쉬어가는 주 행의 `빼고 당기기`를 누릅니다. 그 주를 일정에서 빼고 뒤 회차를 1주씩 당겨 미루기 전 일정으로 돌아갑니다. 2주 미뤘으면 두 주를 각각 뺍니다. 회차로 쓰는 중인 주는 먼저 쉬어가기로 바꿔야 뺄 수 있고, 뒤 회차 중 공지가 이미 나간 회차가 있으면 마감을 앞당길 수 없어 막힙니다.
