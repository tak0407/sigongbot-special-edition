...
#### 시간 확정과 Google Meet 자동 생성

팀별 시간 투표가 끝나면 관리자 웹의 `온라인 회고 확정` 탭에서 시간을 고르고 `시간 확정 및 Meet 생성`을 누릅니다. 확정하면 해당 팀 전용 Google Calendar 이벤트와 Google Meet 링크를 만들고, DB에 기록한 뒤 팀 채널에 `Google Meet 입장`·`출석 체크` 버튼이 달린 확정 공지를 올립니다. 저장된 확정 시간과 Meet 링크는 기존 온라인 회고 진행 스케줄러가 그대로 사용하므로, `ONLINE_RETRO_MEETINGS`를 따로 채우지 않아도 공지·작성·공유·인증샷 흐름이 이어집니다. 두 곳에 같은 회차·팀이 있으면 확정본이 우선합니다.
...
추석이나 설 연휴처럼 일정을 뒤로 밀어야 할 때는 관리자 웹 `회차 일정` 탭 위쪽의 `회차 미루기`를 씁니다. 미룰 첫 회차와 주 수를 고르면 그 회차부터 마지막 회차까지 한꺼번에 밀립니다. 회차 이름과 순서, 회차 사이 간격은 그대로고 기수만 그만큼 늦게 끝나므로, 쉬어간 만큼 뒤에 회차를 더 붙이고 싶으면 `회차 추가`로 추가 회차를 만듭니다. 회차 이름은 회고 행에 그대로 기록되는 값이라 미룰 때도 바꾸지 않습니다.

아직 나가지 않은 제출 공지는 마감과 같은 간격으로 따라 밀립니다. 이미 공지가 나간 회차를 미루면 그 공지 본문에는 예전 마감이 적혀 있으므로 화면에 경고가 뜨고, 채널에는 운영자가 따로 알려야 합니다. 마감이 지난 회차부터는 미룰 수 없습니다. 그 구간에 제출된 회고가 다른 회차에 속한 것처럼 집계되기 때문입니다. 미뤄서 비는 주에는 `추가 회차`가 쉬어가는 주로 생깁니다. 쉬어가는 동안은 마감도 공지도 없고, 그 주에도 회고를 받으려면 표에서 `회차로 쓰기`를 누릅니다. 그러면 공지도 기본 시각(마감 3일 10시간 전)에 나갑니다.

미루기를 되돌리려면 쉬어가는 주 행의 `삭제하기`를 누릅니다. 뒤 회차가 한 주씩 당겨진다는 확인 창이 뜨고, 확인하면 그 주를 일정에서 삭제하고 뒤 회차를 한 주씩 당겨 미루기 전 일정으로 돌아갑니다. 2주 미뤘으면 두 주를 각각 삭제합니다. 회차로 쓰는 중인 주는 먼저 쉬어가기로 바꿔야 삭제할 수 있고, 뒤 회차 중 공지가 이미 나간 회차가 있으면 마감을 앞당길 수 없어 막힙니다.
