...
아직 나가지 않은 제출 공지는 마감과 같은 간격으로 따라 밀립니다. 이미 공지가 나간 회차를 미루면 그 공지 본문에는 예전 마감이 적혀 있으므로 화면에 경고가 뜨고, 채널에는 운영자가 따로 알려야 합니다. 마감이 지난 회차부터는 미룰 수 없습니다. 그 구간에 제출된 회고가 다른 회차에 속한 것처럼 집계되기 때문입니다. 회고 없이 비는 주는 일정 표에 `쉬어가는 주` 줄로 표시되며 Slack에는 아무것도 나가지 않습니다.
